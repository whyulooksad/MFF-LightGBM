...
输入：data/LLM_train/processed/supervised_flows.jsonl
...
def get_or_create_splits(flows=None, force_recreate=False):
    ...
    如果 data/LLM_train/splits/train.jsonl、val.jsonl、test.jsonl 已存在，默认直接读取；
    否则从 supervised_flows.jsonl 创建划分并保存。
    """
    if split_files_exist() and not force_recreate:
        print("发现已保存的固定划分，直接读取 data/LLM_train/splits/*.jsonl")
        return load_splits()

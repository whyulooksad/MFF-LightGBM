...
class Orchestrator:
    ...
    async def submit(self, pcap_path: str, original_filename: str | None = None) -> str:
        task_id = f"T-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:6]}"
        runtime_dir = str(self.runner.task_dir(task_id)) if hasattr(self.runner, "task_dir") else None
        self.store.create_task(task_id, original_filename or pcap_path, runtime_dir=runtime_dir)
        self._queues[task_id] = asyncio.Queue()
        await self._real_lock.acquire()
        coro = self._run(task_id, pcap_path, original_filename)
        self._tasks_in_flight[task_id] = asyncio.create_task(coro)
        return task_id

    async def _run(self, task_id, pcap_path, original_filename):
        async def router(event_name, data):
            await self._queues[task_id].put((event_name, data))
        try:
            await self.runner.run_real(task_id, pcap_path, router, original_filename=original_filename)

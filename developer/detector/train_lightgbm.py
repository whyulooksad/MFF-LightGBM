...
from user_app.inference.contract import ID2LABEL, LABEL2ID
...
from user_app.inference.supcon_reducer import SupConAEReducer, replace_semantic_features, semantic_feature_columns
...
CHECKPOINT_DIR = EXPERIMENT_DETECTOR_DIR
...
IMPUTATION_MEDIANS_JSON = CHECKPOINT_DIR / "imputation_medians.json"
REPORT_DIR = EVALUATION_REPORTS_DIR / "detector_report"
FIGURES_DIR = EVALUATION_FIGURES_DIR
SUPCON_MODEL_PATH = EXPERIMENT_SUPCON_MODEL_PATH
...
def preprocess_detector_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    non_num_cols = df.select_dtypes(include=["object", "string"]).columns.tolist()
    non_num_cols = [col for col in non_num_cols if col not in protected]
    if non_num_cols:
        raise ValueError(
            f"检测器输入包含契约外的非数值列 {non_num_cols}；"
            "禁止在全数据上临时拟合 LabelEncoder"
        )
...
def save_imputation_medians(features: pd.DataFrame) -> None:
    """Persist training-time medians so pure inference can impute identically."""
    medians = features.median(numeric_only=True)
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    IMPUTATION_MEDIANS_JSON.write_text(
        json.dumps({key: float(value) for key, value in medians.items()}, ensure_ascii=False),
        encoding="utf-8",
    )
...
def write_test_outputs(test_df: pd.DataFrame, y_test: np.ndarray, proba: np.ndarray) -> pd.DataFrame:
    ...
    EVALUATION_PREDICTIONS_DIR.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(EVALUATION_PREDICTIONS_DIR / "predictions.csv", index=False)
    ...
    ).to_csv(EVALUATION_REPORTS_DIR / "confusion_matrix.csv")
    ...
    (EVALUATION_REPORTS_DIR / "classification_report.txt").write_text(report, encoding="utf-8")
    print(report)
    return out_df
...
def write_weighted_metrics_plot(results_df: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "weighted_metrics_comparison.png", dpi=200)
    plt.close()
...
def main(
    ...
    train_idx, val_idx, test_idx = predefined_split_indices(raw_df)
    y_train_lgb, y_val_lgb, y_test = y[train_idx], y[val_idx], y[test_idx]
    ...
    if reducer_name == "supcon":
        ...
        reducer = SupConAEReducer.load(SUPCON_MODEL_PATH, verbose=True)
        if reducer.feature_columns != semantic_cols:
            raise ValueError("已训练 SupCon-AE 的输入列与当前语义特征不一致")
        ...
        df = reduce_feat_in_memory(df, n_components=n_components, seed=SEED, fit_indices=train_idx)
    ...
    # Fit imputation using training groups only. Columns with no training
    # observation cannot be learned and are excluded from the released model.
    train_medians = df.iloc[train_idx][feature_cols].median(numeric_only=True)
    feature_cols = [column for column in feature_cols if pd.notna(train_medians.get(column))]
    if not feature_cols:
        raise ValueError("训练集合中所有候选特征均为缺失值")
    train_medians = train_medians[feature_cols]
    df[feature_cols] = df[feature_cols].fillna(train_medians)
    save_feature_columns(feature_cols)
    save_imputation_medians(df.iloc[train_idx][feature_cols])
    ...
    print(f"  confusion_matrix_png={FIGURES_DIR / 'confusion_matrix.png'}")
    print(f"  weighted_metrics_png={FIGURES_DIR / 'weighted_metrics_comparison.png'}")
    print(f"  roc={FIGURES_DIR / 'roc_curve.png'}")

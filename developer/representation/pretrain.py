...
输入：
    data/developer/workspace/corpus/pretrain/pretrain_flows.jsonl，每行包含一条流的 text。
...
def load_rtd_texts(jsonl_path=None, holdout_ratio=0.1):
    """从外层 train 中按 group_id 固定切出 RTD 内部验证组。"""
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if text:
                if not group_id:
                    raise ValueError("RTD语料缺少 group_id，不能安全创建内部验证集")
                records.append((group_id, text))
            lbl = obj.get("label")
            label_counts[lbl] = label_counts.get(lbl, 0) + 1

    groups = sorted(
        {group for group, _ in records},
        key=lambda value: hashlib.sha256(f"rtd-holdout|{value}".encode("utf-8")).hexdigest(),
    )
    if len(groups) < 2:
        raise ValueError("RTD至少需要两个独立train来源组，才能创建内部holdout")
    holdout_count = min(len(groups) - 1, max(1, round(len(groups) * holdout_ratio)))
    holdout_groups = set(groups[:holdout_count])
    train_texts = [text for group, text in records if group not in holdout_groups]
    holdout_texts = [text for group, text in records if group in holdout_groups]
    print(f"加载 {len(records):,} 条流文本")
    print(f"  label 分布: {label_counts}")
    print(f"  RTD fit={len(train_texts):,}, holdout={len(holdout_texts):,}, holdout_groups={len(holdout_groups):,}")
    return train_texts, holdout_texts


@torch.no_grad()
def evaluate_rtd(model, dataloader, tokenizer, device):
    model.eval()
    losses = []
...
def pretrain(jsonl_path=None, epochs=None, batch_size=None, lr=None):
    ...
    print("=" * 60)
    print("[1/5] 加载数据")
    train_texts, holdout_texts = load_rtd_texts(jsonl_path)
    if not train_texts or not holdout_texts:
        raise ValueError("没有可用于预训练的文本，请先生成 data/developer/workspace/corpus/pretrain/pretrain_flows.jsonl")
    ...
    dataset = LazyFlowTextDataset(train_texts, tokenizer, PRETRAIN_MAX_LENGTH)
    holdout_dataset = LazyFlowTextDataset(holdout_texts, tokenizer, PRETRAIN_MAX_LENGTH)
    ...
    holdout_loader = DataLoader(holdout_dataset, batch_size=batch_size, shuffle=False, drop_last=False)
    print(f"  train batches={len(dataloader)}, holdout batches={len(holdout_loader)} (batch_size={batch_size})")
    ...
    for epoch in range(1, epochs + 1):
        ...
        holdout_loss = evaluate_rtd(model, holdout_loader, tokenizer, device)
        print(f"  Epoch {epoch} 完成: train_loss={avg_loss:.4f}, holdout_loss={holdout_loss:.4f}")

        improved = holdout_loss < best_loss - PRETRAIN_MIN_DELTA
        if improved:
            ...
            if stale_epochs >= PRETRAIN_EARLY_STOP_PATIENCE:
                print("  -> 触发RTD holdout早停")
                break

    print("\n[5/5] RTD 继续预训练完成")
    print(f"  最佳 holdout loss: {best_loss:.4f}")

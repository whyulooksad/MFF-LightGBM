...
输入：
    1. data/developer/workspace/corpus/supervised/supervised_flows.jsonl
    2. models/experiments/current/encoder/checkpoint-epoch*/ 中的RTD继续预训练encoder

输出：
    models/experiments/current/lora/best/ 下的LoRA adapter和tokenizer。
"""
...
def train_lora_classifier(
    ...
    if base_model_dir is None:
        ...
        if base_model_dir is None:
            raise FileNotFoundError(
                "未找到RTD预训练checkpoint。请先运行 python -m developer.representation.pretrain，"
    ...
    train_flows, val_flows, _ = materialize_authoritative_splits(flows=flows)
    ...
    print(f"  train batches={len(train_dl)}, val batches={len(val_dl)}; test remains sealed")
    ...
    if not os.path.exists(best_dir):
        raise RuntimeError("LoRA 未产生验证集最优 checkpoint")
    print("  test 集保持封存；最终组合模型完成后再统一评估")

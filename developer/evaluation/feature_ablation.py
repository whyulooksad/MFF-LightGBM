...
from user_app.inference.contract import ID2LABEL
...
REPORT_DIR = EVALUATION_REPORTS_DIR / "detector_report" / "feature_ablation"
CHECKPOINT_DIR = EXPERIMENT_DIR / "detector" / "feature_ablation"
BATCH_SIZE = 4096
...
def split_indices(frame: pd.DataFrame, y: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    train_idx, val_idx, test_idx = predefined_split_indices(frame)
...
def plot_metrics(metrics_df: pd.DataFrame) -> None:
    ...
    plt.savefig(EVALUATION_FIGURES_DIR / "feature_ablation_metrics.png", dpi=200)
    plt.close()
...
def plot_roc(y_test: np.ndarray, proba_by_group: dict[str, np.ndarray]) -> None:
    ...
    plt.tight_layout()
    EVALUATION_FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(EVALUATION_FIGURES_DIR / "feature_ablation_roc.png", dpi=200)
    plt.close()
...
def main(n_components: int = 64):
    ...
    train_idx, val_idx, test_idx, y_train, y_val = split_indices(raw_df, raw_y)

    semantic_cols = semantic_feature_columns(raw_df)
    print(f"      SupCon-AE 降维 feat_*: {len(semantic_cols)} -> {n_components} 维 (manual 原样保留)")
    reducer = SupConAEReducer.load(EXPERIMENT_SUPCON_MODEL_PATH, verbose=True)
    if reducer.feature_columns != semantic_cols:
        raise ValueError("消融输入与已训练 SupCon-AE 的特征列不一致")
    if int(reducer.config["latent_dim"]) != n_components:
        raise ValueError(
            f"消融请求 {n_components} 维，但已训练 reducer 为 {reducer.config['latent_dim']} 维"
        )

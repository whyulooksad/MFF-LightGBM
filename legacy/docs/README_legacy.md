...
- `data/developer/datasets`：官网下载的原始数据、标签清单和准备后的离线数据；
- `data/developer/workspace`：开发者训练、评估和可再生成的中间产物；
- `data/runtime`：用户上传 PCAP 后生成的任务级临时文件与结果；
- `models/experiments/current`：离线训练产生的候选模型；
- `models/production/v1`：Web 推理允许加载的已发布模型。

`developer.pipeline` 是开发者离线入口。用户上传不会调用它，而是由后端调用
`user_app.inference.pipeline`。官网下载的压缩包应原样放入
`data/developer/datasets/source/<dataset>/archives`，解压到相邻的 `extracted`，不能直接放入
`data/developer/datasets/prepared`。

`developer` 只负责数据准备、训练和评估；`user_app` 只负责用户推理、后端和前端。
训练与推理必须一致的解析和特征逻辑由开发侧直接复用 `user_app/inference`，反向依赖被禁止。
完整目录说明见
